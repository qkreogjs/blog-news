    ...
    <div>
      ...
      {lead ? (
            ...
            <div className="md:col-span-3">
              <Link href={`/blog/${lead.slug}`} className="group block">
                ...
                <div style={{ color: "#6B7280" }} className="text-xs flex items-center gap-3">
                  <time>{new Date(lead.publishedAt).toLocaleDateString("ko-KR")}</time>
        ...
        <div className="text-center py-20">
          <p style={{ color: "#9CA3AF" }}>첫 번째 기사가 곧 올라옵니다.</p>
        </div>
      )}
              ...
              {secondary.map(post => (
                <article key={post.slug} style={{ borderTop: "2px solid #111827" }} className="pt-3 group">
                  <Link href={`/blog/${post.slug}`}>
                    ...
                    <time style={{ color: "#9CA3AF" }} className="text-xs">{new Date(post.publishedAt).toLocaleDateString("ko-KR")}</time>
                  </Link>
                </article>
              ))}
            </div>
              ...
              <article key={post.slug} style={{ borderBottom: "1px solid #E5E7EB" }} className="py-3 group">
                <Link href={`/blog/${post.slug}`} className="flex gap-3 items-start">
                  ...
                  <div>
                    ...
                    <time style={{ color: "#9CA3AF" }} className="text-xs">{new Date(post.publishedAt).toLocaleDateString("ko-KR", { month: "2-digit", day: "2-digit" })}</time>
                  </div>
                </Link>
              </article>
            ))}
            {listPosts.length === 0 && secondary.length === 0 && lead === undefined && (
              <p style={{ color: "#9CA3AF" }} className="text-xs py-4">기사가 없습니다.</p>
            )}
          </div>
          <a href="/blog" style={{ color: "#C8000A", borderTop: "1px solid #E5E7EB" }} className="block pt-3 text-xs font-bold hover:opacity-80">전체 기사 보기 →</a>
        </div>
      </div>
    </div>
  );
